...
const int32_t dtcf16[NDTCF16][5] =
{
	...
	/*01*/{ 2916171, 32589651, -12743920,  1198080,        0 }, /* ybeg=-1320, yend= -720 用线性插值过渡到长期估算公式结果*/
	...
	/*56*/{ 4916969,    69737,      1782,     -739,       88 }, /* ybeg= 2016, yend= 2019 根据当前数据自行拟合 */
	/*57*/{ 4923299,    70881,      5796,    -2496,     1287 }, /* ybeg= 2019, yend= 2027.75 根据预测数据自行拟合 */
};
...
int32_t calculate_deltaT(jd_t julian_day)
{
	...
	*/
	...
	/*
	-720之前及表末尾年份之后，使用长期估算公式
	-1320~-720通过线性插值方式过渡，已合入多项式表中。
	*/
	if ((jdx2 < dtcf16[0][0]) || (jdx2 >= dtcf16[NDTCF16 - 1][0]))
	{
		...
		/*
		保证连续性，在表结尾之后，过渡到长期估算公式结果，过渡期为CALCULATE_DELTAT_JD_X2_INTERVAL。
		*/
